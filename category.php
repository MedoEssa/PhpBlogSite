...
$categories_query = "SELECT * FROM categories WHERE cat_id = " . escape_string($the_category_id) . " ";
    ...
    while ($row = fetch_array($send_categories_query)) {
        ...
        echo "<h2 class='category_title'><span class='theme-color'>{$cat_title}</span></h2>";
     }
						...
						<div class="row">
							...
							if (isset($_GET['cat_id'])) {
							 ...
							 if (isset($_GET['page'])) {
			                  ...
			                  $query = "SELECT * FROM posts WHERE post_category_id = " . escape_string($the_category_id) . " ";
			                  ...
			                  $count = ceil($count / 2);
							    ...
							    // Display all Posts
							    $query = "SELECT * FROM posts WHERE post_category_id = {$the_category_id} LIMIT $page_1, 2 ";
								$send_query = query($query);
								confirm($send_query);
								...
								if (mysqli_num_rows($send_query) < 1) {
									...
									echo "<h4> NO POSTS IN <span class='category-title'>{$cat_title} </span> CATEGORY </h4>";
												...
												<header class="entry-header">
													...
													<div class="entry-meta">
														...
														<span class="byline meta-span">
															...
															<span>
																<a href="/blog/author.php?author=<?php echo $post_user; ?>"><?php echo $post_user; ?></a>
															</span>
														</span>
							...
							for($i=1;$i<=$count;$i++) {
								...
								if ($page == $i) {
									 echo "<li><a class='active-link' href='/blog/category/{$the_category_id}/{$i}'>{$i}</a></li>";
								} else {
									 echo "<li><a href='/blog/category/{$the_category_id}/{$i}'>{$i}</a></li>";
								}
							...
							<!-- PAGINATION SYSTEM END -->
	                          ...
	                          <!-- SIDEBAR FOR CATEGORY -->
                                ...
                                <?php include "includes/sidebar_category.php"; ?>					
                        </div><!-- end row -->
					</main>
				</div>
